...
def answer_valid(answer):
    if len(answer) == 1 or len(answer) == len(current_word):
        for i in answer:
...
def play_again():  # сыграем еще?
    print('\nСыграем еще раз? Введите: да/нет\n')
    play_again_answer = input()
    while 'да' not in play_again_answer.lower() and 'нет' not in play_again_answer.lower():
        print('Не понятно. Да или Нет?')
        play_again_answer = input()
    if 'нет' in play_again_answer.lower():
        print('ОК')
    elif 'да' in play_again_answer.lower():
        play_hangman()
...
def play_hangman():
    ...
    word_completion = '_' * len(current_word)  # строка, содержащая символы _ на каждую букву задуманного слова
    guessed_letters = []               # список уже названных букв
    guessed_words = []                 # список уже названных слов
    tries = 6                          # количество попыток

    print('Давайте сыграем в угадайку слов!')
    print('Ваша задача - отгадать слово до того, как будет дорисован человечек на виселеце.\nВы можете вводить буквы (по одной за раз) или слово целиком.\nУ вас есть 6 попыток.')
    time.sleep(4)
    print()
    print('Итак, слово загадано')
    print(display_hangman(tries))
    print(f'В слове {len(current_word)} букв:', word_completion)

    while True:

        answer = input(f'Введите букву или слово целиком: ').upper()

        if answer_valid(answer):
            if len(answer) == len(current_word):  # введено слово
                if answer == current_word:  # слово угадано
                    print('Да! Вы выиграли! Поздравляем!')
                    break
                elif answer in guessed_words:  # слово уже называлось
                    print('Вы уже называли это слово')
                else:  # не правильное слово
                    print('Нет, это другое слово')
                    guessed_words.append(answer)
                    tries -= 1
            elif len(answer) == 1:  # названа буква
                if answer in guessed_letters:  # буква уже называлась
                    print('Вы уже называли эту букву')
                elif answer in current_word:  # буква есть в слове, раньше не называлась
                    print('Есть такая буква!')
                    guessed_letters.append(answer)  # добавляем в список называвшихся
                    for i in range(len(current_word)):
                        if current_word[i] == answer:
                            word_completion = word_completion[:i] + answer + word_completion[i + 1:]
                elif answer not in current_word:
            ...
            print(
                'Введено некорректное значение. Можно вводить русские буквы (по одной за раз)\nили слово целиком (количество букв в слове должно совпадать с количеством в загаданном слове)')
        print(display_hangman(tries))
        ...
        if tries == 0:
            ...
            print('Да! Вы отгадали слово! Поздравляем!')
            break
    play_again()
play_hangman()